...
from ranking import *
from kNN import load_mssg_senses, load_hsm_senses, avgSim

if __name__=='__main__':  
  word_vec_file = sys.argv[1]
  word_sim_dir = sys.argv[2]
  
  # word_vecs = read_word_vectors(word_vec_file)  # original
  # word_vecs = load_mssg_senses(word_vec_file)

  vocab_file = sys.argv[3]  # only used for hsm
  word_vecs = load_hsm_senses(vocab_file, word_vec_file)

  # normalize word vectors? need to do it for each sense
  # word_vecs[word][i] /= math.sqrt((word_vecs[word][i]**2).sum() + 1e-6)
  print '================================================================================='
  ...
  for i, filename in enumerate(os.listdir(word_sim_dir)):
    ...
    for line in open(os.path.join(word_sim_dir, filename),'r'):
      ...
      if word1 in word_vecs and word2 in word_vecs:
        ...
        auto_dict[(word1, word2)] = avgSim(word_vecs[word1], word_vecs[word2])
      else:
        not_found += 1
      total_size += 1    
