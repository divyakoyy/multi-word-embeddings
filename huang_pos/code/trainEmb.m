...
corpusFileName = '../data/test/corpus/';
numFiles = 1;      % number of files in corpusFileName
...
% maps word id to the word 
vocab = cell(1,length(fileLines)-1);
% maps word id to the frequency
freq = zeros(1,length(fileLines)-1);
for i = 1:length(fileLines)
    tempstr = fileLines{i};
    temp=regexp(tempstr,' ','split');
    word_id_temp = strsplit(temp{2}, '_');
    word_id = word_id_temp{1};
    word_temp = strsplit(temp{1}, '_');
    word = char(word_temp{1});
    if ischar(word)
        % maps word id to the word 
        vocab{str2num(word_id)} = word;
        freq(str2num(word_id)) = str2num(temp{3});
    end
end
